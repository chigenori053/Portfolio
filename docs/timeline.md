...
**目的：中学生以上の学生に向けた、数学学習支援プロダクト。**

研究のための実験言語ではなく、**学習支援ツール**として始まっています。
プログラミング学習と数学学習を同時に支援する Python ベースの DSL として設計されました。

そのために解く必要があったのが、この問いです。
...
学習支援ツールとして必須だったのが、**教科書どおりの記法を受け取ること**です。
`MathLangInputParser` が `x^2` → `x**2`、`2xy` → `2*x*y`、`√x` → `sqrt(x)` のように
人間の数式表記を内部形式へ正規化し、仕様書はその狙いを
*"hides Python/SymPy-like syntax from educational users"* と明記しています。

そのうえで **SymPy をランタイムに組み込み**（`SymbolicEngine`）、
`ValidationEngine` が学習者の解答を3つのモードで判定します。

| モード | 判定 |
|---|---|
| `symbolic_equiv` | 数学的に同値なら正解 — **形の異なる複数の解答を受理** |
| `exact_form` | 正規化後に完全一致 |
| `canonical_form` | 指定された標準形に一致 |

「数学的には正しいが、求められた形ではない」を区別できる点が、
単なる正誤判定を超えた学習支援になっています。
誤答時には `CausalEngine` が原因を推定し、**修正候補を最大3件**提示します。
