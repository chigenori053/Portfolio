...
2025年1月のリサーチ期から2026年8月までの約1年7ヶ月間の流れです。
各リポジトリはコミット履歴が単一のため、日付は GitHub のリポジトリ作成日・更新日、
および仕様書・CHANGELOG の記載日に基づいています。2025年1月〜10月のリサーチ期は
コードを伴わない検討・相談が中心だったため、GitHub 上に記録はありません。
...
```
2025-01 ──── リサーチ期              「学習コーチとしてLLMを活用できないか？」
            │                       専門家相談・SymbolicAI・Wolfram Alpha検証
            │                       → 「途中計算を含む数式を理解できる教育言語」
            ▼
...
**この期間はコードを伴わないため、GitHub上には残っていません。**
運営するプログラミング教室で数学学習コースの新設を検討する中で生まれた着想を、
専門家への相談と既存サービスの検証を通じてコンセプトへと固めていった期間です。

1. 運営するプログラミング教室で、**数学学習コースの新設**を検討。
2. その中で、**学習コーチとして LLM を活用する**アイディアに着想。ただし当時の LLM は
   数学計算そのものに問題が残り、生成されるアウトプットも安定していなかった。
3. そこで教育の専門家に相談したところ、**「難しい」**というフィードバックを受けた。
4. その後 **SymbolicAI** の存在を知る。
5. 続けて、教育分野の既存サービスとして **Wolfram Alpha** を知り、実際にテスト。
   その結果、実現したかったこと——**入力された数式に対して、途中計算を含めた段階的な
   評価を行うこと**——ができないと判明した。
6. ここから、**MathLang のコンセプト**（途中計算式を含む数式を理解・評価できる、
   専用の教育言語）に到達した。

この結論をそのまま実装へ落としたものが、次節の mathlang です。
