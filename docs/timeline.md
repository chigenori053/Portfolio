...
**問い：中学生以上の学習者が使う数式を、Python 上で数学的に判定しながら、
思考の「過程」を検査・再生できるデータとして書けるか？**
...
人間向けの `x^2`・`2xy`・`√x`・隣接括弧は入力パーサが Python / SymPy 互換表記に正規化します。
SymPy による同値判定では、同じ問題に対する複数の等価な回答形式を受け入れます。
